...
- 2026-08-03 — MCP/Cursor OAuth against Cognito: **end-to-end connection confirmed working, Cursor shows tools available.** Getting there surfaced two separate, unrelated Cognito-specific gaps, easy to conflate because both manifested as opaque auth failures with no useful client-side error text:
  - **Symptom 1 — token exchange itself failing (`invalid_grant`).** Root cause: Cognito's token endpoint rejects the RFC 8707 `resource` parameter that MCP clients send unconditionally on authorization/token requests (confirmed via a standalone PKCE debug script, `scripts/oidc_pkce_debug.py`, comparing requests with/without `resource`). Entra ID v2.0 has the same gap (`AADSTS9010010`), so switching `MCP_OAUTH_ISSUER` to Entra would not have helped, and there is no Cursor-side setting to omit `resource` (it's MCP-spec-mandated). No gate-side code change fixes this directly — the resolution path in the wild is a stripping proxy in front of the AS's token endpoint (what the `agentgateway` project does for the same Entra gap). This did stop blocking without a gate change (plausibly because Cursor's *refresh*-token grant, unlike the initial code exchange, didn't carry `resource` the same way — not fully root-caused, so treat this as fragile rather than durably fixed).
