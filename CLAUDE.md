...
- `backend/app/api/mcp_gateway.py` — `build_mcp_app()`: the `/mcp` Streamable HTTP route (official `mcp` SDK's `MCPServer`), tools `create_change_ticket` / `check_ticket_status` / `get_change_ticket_details` / `supersede_change_ticket` / `close_ticket` / `describe_operation_parameters`. The two read tools are a deliberate pair and their docstrings are written against each other — tool descriptions are the only routing signal the model gets, so `check_ticket_status` is scoped to "is it approved yet" and `get_change_ticket_details` to "what happened / what did it create". Both go through `_own_ticket`, so the ownership rule can't drift between them. Adding or renaming a tool is invisible in Cursor until the server key in its MCP config is renamed — it never re-issues `tools/list`. Mounted at the ASGI level in `main.py`, not via `app.include_router` — see the comment there for why (the SDK's own auth middleware and `main.py`'s SPA catch-all route can't cleanly nest inside the same FastAPI router).
