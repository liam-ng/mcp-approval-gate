...
- `aws_schema.py` — validates a proposed call against **botocore's own service models** at creation time (`build_ticket`, so all three creation paths get it), rejecting unknown operations/parameters, wrong types and missing required members with 422 `INVALID_ACTION_PARAMETERS` before an approver is ever asked. Reads botocore's local JSON only — **never construct a boto3 client here**, that starts credential resolution and would break the "gate needs no AWS permissions" invariant. Known limit: the models can't express conditional requirements (`RunInstances` needs `ImageId` unless `LaunchTemplate`, but lists neither as required), so this is a floor, not a guarantee — a DryRun pre-flight from the executor is the planned second layer.
